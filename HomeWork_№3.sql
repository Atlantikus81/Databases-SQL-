...
INSERT INTO `friend_requests` (`initiator_user_id`, `target_user_id`, `status`, `requested_at`, `confirmed_at`) VALUES ('2', '3', 'approved', '2015-02-07 01:53:07', '2015-09-14 16:17:12');
INSERT INTO `friend_requests` (`initiator_user_id`, `target_user_id`, `status`, `requested_at`, `confirmed_at`) VALUES ('2', '1', 'approved', '1983-04-08 15:57:26', '1976-12-28 23:54:58');
...
SELECT initiator_user_id, COUNT(*) AS count_max
FROM friend_requests
GROUP BY initiator_user_id
ORDER BY count_max DESC
LIMIT 1;
-- Немного подкорректировал Вашу базу данных ради эксперимента.
-- В новом варианте одним пользователем отправлено максимум 3 заявки в друзья, его id = 2.
-- Долго не получалось вывести id пользователя, отправившего максимальное количество заявок.
-- Оказалось, нужно было обновить всю базу данных))) Всего-то)))
